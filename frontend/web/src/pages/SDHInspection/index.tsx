...
const soNames = [
  'Shevgaon SO', 'Pathardi SO', 'Kharwandi kasar SO', 'Miri SO', 
  'VSSK SO', 'Tisgaon SO', 'Bodhegaon SO', 'Balamtakali SO'
];
...
const SDHInspection = () => {
  ...
  const fetchInspections = async () => {
    try {
      setLoading(true);
      const response = await apiClient.get('/sdhinspections');
  ...
  const confirmDelete = async () => {
    ...
    try {
      await apiClient.delete(`/sdhinspections/${deleteItem._id || deleteItem.id}`);
      setDeleteItem(null);
      fetchInspections();
    } catch (err) {
  ...
  const columns = [
    ...
    { key: 'result', header: 'Result', render: (val: string) => val || 'N/A' },
    ...
    <div className="animate-fade-in" style={{ padding: '24px' }}>
      ...
      <Modal isOpen={showModal} onClose={() => { setShowModal(false); setEditingItem(null); }} title={editingItem ? "Edit Inspection" : "Add New Inspection"} width="640px">
        <form onSubmit={async (e) => {
          ...
          try { 
            if (editingItem) {
              await apiClient.put(`/sdhinspections/${editingItem._id || editingItem.id}`, payload);
            } else {
              await apiClient.post('/sdhinspections', payload); 
            }
            setShowModal(false); 
            setEditingItem(null);
            fetchInspections(); 
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            ...
            <div style={{ gridColumn: 'span 2' }}>
              <FormField label="Result" name="result" defaultValue={editingItem?.result} />
            </div>
          </div>
